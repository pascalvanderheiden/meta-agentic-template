...
- **Primary:** GitHub MCP server (configured in `.copilot/mcp-config.json`, server name `github`)
- **Fallback:** `gh api repos/{owner}/{repo}/issues` (via `gh` CLI) per the github-issues skill
